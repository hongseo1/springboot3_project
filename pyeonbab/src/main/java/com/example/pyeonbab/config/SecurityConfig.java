package com.example.pyeonbab.config;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.formLogin(form -> form
                .loginPage("/index#login_page") //로그인 페이지 URL 설정
                .defaultSuccessUrl("/") //로그인 성공 시 이동할 URL 설정
                .usernameParameter("email") //로그인 시 사용할 파라미터 이름으로 email을 지정
                .failureUrl("/index#login_page/error") //로그인 실패 시 이동할 URL을 설정
        ).logout(logout -> logout
                .logoutRequestMatcher(new AntPathRequestMatcher("/index/logout"))//로그 아웃 URL을 설정
